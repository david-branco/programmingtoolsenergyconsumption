...
static inline long vec_nle(__m128d *v, double f)
{
    return (v[0][0] <= f ||
        v[0][1] <= f ||
        v[1][0] <= f ||
...
static inline void clrPixels_nle(__m128d *v, double f, unsigned long * pix8)
{
...
static inline void calcSum(__m128d *r, __m128d *i, __m128d *sum, __m128d *init_r, __m128d init_i)
{
...
int main(int argc, char ** argv)
{
    ...
    int filedesc = open("output", O_WRONLY|O_CREAT|O_TRUNC, 0644);
    if (filedesc < 0) {
        return -1;
    }
    /*********************************/   
    ...
    long ret = ret = write(filedesc, header, dataLength);
    free(buffer);
